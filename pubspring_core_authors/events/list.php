...
<div id="tribe-events-content" class="upcoming">
	...
	<?php while ( have_posts() ) : the_post(); ?>
		...
		<div id="post-<?php the_ID(); ?>" <?php post_class('tribe-events-event clearfix'); ?> itemscope itemtype="http://schema.org/Event">
			...
			<?php endif; ?>
			<?php the_title('<h2 class="entry-titleTK" itemprop="name"><a href="' . tribe_get_event_link() . '" title="' . the_title_attribute('echo=0') . '" rel="bookmark">', '</a></h2>'); ?>
			...
			<div class="entry-content tribe-events-event-entry" itemprop="description">
				...
				<?php else: ?>
					<?php the_content(); ?>
				<?php endif; ?>
				<div style="margin-top: 6px;"><?php  do_action ('ps_sharing_posts');    ?></div>
			...
			<div class="tribe-events-event-list-meta" itemprop="location" itemscope itemtype="http://schema.org/Place">
				<table cellspacing="0">
					...
					<?php  $tribe_start_date = tribe_get_start_date();
							$tribe_end_date = tribe_get_end_date();
							
							
						if ($tribe_start_date !== $tribe_end_date) { ?>
