...
<?php if( $related_reviews ): ?>
	<?php foreach( $related_reviews as $related_review ):  $count_related++;?>	
		...
		<?php if ( $count_related == 2   ){echo '<a class="badge show_hidden_' .get_the_ID().' pull-right" style="margin-top:-30px;" href="javascript:void(0)">
		<span class="show_text">see more reviews</span>
		<span class="hidden_stuff">hide reviews</span>		
		</a>
		
		';}?>	
<div <?php if ( $count_related > 1   ){echo 'class="hidden_stuff hidden_' .get_the_ID().'" style="margin-top:3em;"';}?>>
