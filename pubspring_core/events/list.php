	...
	<?php while ( have_posts() ) : the_post(); ?>
		...
		<div id="post-<?php the_ID() ?>" <?php post_class('tribe-events-event clearfix') ?> itemscope itemtype="http://schema.org/Event">
			...
			<?php if (tribe_get_organizer()): ?>
				<h3><?php echo tribe_get_organizer(); ?></h3>
			<?php endif; ?>
			...
			<div class="tribe-events-event-list-meta" itemprop="location" itemscope itemtype="http://schema.org/Place">
				...
				<table cellspacing="0">
					...
					<tr>
					<?php //BPNY changed this row to show date and added formatting ?>
						<td class="tribe-events-event-meta-desc"><?php _e('Day:', 'tribe-events-calendar') ?></td>
						<td class="tribe-events-event-meta-value" itemprop="startDate" content="<?php echo tribe_get_start_date(); ?>"><?php echo tribe_get_start_date($post->ID, false, 'D, M jS'); ?></td>
					</tr>
					
					<tr>
					<?php //BPNY changed start to time and added start date formatting ?>
						<td class="tribe-events-event-meta-desc"><?php _e('Time:', 'tribe-events-calendar') ?></td>
						<td class="tribe-events-event-meta-value" itemprop="startDate" content="<?php echo tribe_get_start_date(); ?>"><?php echo tribe_get_start_date($post->ID, false, 'g:i a'); ?></td>
					</tr>
			
			<?php //BPNY don't show below ?>
					<tr class="hidden">
			...
			<?php //don't show above ?>			
					...
					<?php //BPNY added to show link - if we have an organizer name, use that for the link text, otherwise use "event Link" ?>
					<?php if ( tribe_get_organizer_link( get_the_ID(), false, false ) ) : ?>
					<tr>
						<td class="tribe-events-event-meta-desc"><?php _e('Info:', 'tribe-events-calendar') ?></td>
						<td><a href='<?php echo tribe_get_organizer_link(get_the_ID(),false); ?>' title="Link to more information"><?php if (tribe_get_organizer()): ?><?php echo tribe_get_organizer(); ?><?php else: ?>Event Link<?php endif; ?></a></td>
					 </tr>
					<?php endif; //END show link ?>
					<?php
						$cost = tribe_get_cost();
						if ( !empty( $cost ) ) :
					?>
